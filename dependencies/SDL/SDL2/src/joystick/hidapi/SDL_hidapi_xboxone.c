...
static SDL_bool
HIDAPI_DriverXboxOne_IsSupportedDevice(Uint16 vendor_id, Uint16 product_id, Uint16 version,
...
static SDL_bool
HIDAPI_DriverXboxOne_Init(SDL_Joystick *joystick, hid_device *dev, Uint16 vendor_id,
    ...
    *context = ctx;
    ...
    for (i = 0; i < SDL_arraysize(xboxone_init_packets); ++i) {
        ...
        if (!packet->vendor_id ||
            (vendor_id == packet->vendor_id && product_id == packet->product_id)) {
...
static int
HIDAPI_DriverXboxOne_Rumble(SDL_Joystick *joystick, hid_device *dev, void *context,
                            Uint16 low_frequency_rumble, Uint16 high_frequency_rumble,
                            Uint32 duration_ms) {
    SDL_DriverXboxOne_Context *ctx = (SDL_DriverXboxOne_Context *) context;
    Uint8 rumble_packet[] = {0x09, 0x00, 0x00, 0x09, 0x00, 0x0F, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x00,
                             0xFF};
...
static void
HIDAPI_DriverXboxOne_HandleModePacket(SDL_Joystick *joystick, hid_device *dev,
                                      SDL_DriverXboxOne_Context *ctx, Uint8 *data, int size) {
    if (data[1] == 0x30) {
        /* The Xbox One S controller needs acks for mode reports */
        const Uint8 seqnum = data[2];
        const Uint8 ack[] = {0x01, 0x20, seqnum, 0x09, 0x00, 0x07, 0x20, 0x02, 0x00, 0x00, 0x00,
                             0x00, 0x00};
        hid_write(dev, ack, sizeof(ack));
    }

    SDL_PrivateJoystickButton(joystick, SDL_CONTROLLER_BUTTON_GUIDE,
                              (data[4] & 0x01) ? SDL_PRESSED : SDL_RELEASED);
}
