...
HIDAPI_UpdateDiscovery() {
    if (!SDL_HIDAPI_discovery.m_bCanGetNotifications) {
        ...
        if (!SDL_HIDAPI_discovery.m_unLastDetect || SDL_TICKS_PASSED(now,
                                                                     SDL_HIDAPI_discovery.m_unLastDetect +
                                                                     SDL_HIDAPI_DETECT_INTERVAL_MS)) {
            SDL_HIDAPI_discovery.m_bHaveDevicesChanged = SDL_TRUE;
...
HIDAPI_GetDeviceDriver(SDL_HIDAPI_Device *device) {
    ...
    if (device->usage && device->usage != USAGE_JOYSTICK && device->usage != USAGE_GAMEPAD &&
    ...
    for (i = 0; i < SDL_arraysize(SDL_HIDAPI_drivers); ++i) {
        SDL_HIDAPI_DeviceDriver *driver = SDL_HIDAPI_drivers[i];
        if (driver->enabled &&
            driver->IsSupportedDevice(device->vendor_id, device->product_id, device->version,
...
static void SDLCALL
SDL_HIDAPIDriverHintChanged(void *userdata, const char *name, const char *oldValue,
                            const char *hint) {
    int i;
    SDL_HIDAPI_Device *device = SDL_HIDAPI_devices;
    SDL_bool enabled = (!hint || !*hint ||
                        ((*hint != '0') && (SDL_strcasecmp(hint, "false") != 0)));
...
HIDAPI_AddDevice(struct hid_device_info *info) {
    ...
    if (!device->name && info->manufacturer_string && info->product_string) {
        char *manufacturer_string = SDL_iconv_string("UTF-8", "WCHAR_T",
                                                     (char *) info->manufacturer_string,
                                                     (SDL_wcslen(info->manufacturer_string) + 1) *
                                                     sizeof(wchar_t));
        char *product_string = SDL_iconv_string("UTF-8", "WCHAR_T", (char *) info->product_string,
                                                (SDL_wcslen(info->product_string) + 1) *
                                                sizeof(wchar_t));
        if (!manufacturer_string && !product_string) {
            if (sizeof(wchar_t) == sizeof(Uint16)) {
                manufacturer_string = SDL_iconv_string("UTF-8", "UCS-2-INTERNAL",
                                                       (char *) info->manufacturer_string,
                                                       (SDL_wcslen(info->manufacturer_string) + 1) *
                                                       sizeof(wchar_t));
                product_string = SDL_iconv_string("UTF-8", "UCS-2-INTERNAL",
                                                  (char *) info->product_string,
                                                  (SDL_wcslen(info->product_string) + 1) *
                                                  sizeof(wchar_t));
            } else if (sizeof(wchar_t) == sizeof(Uint32)) {
                manufacturer_string = SDL_iconv_string("UTF-8", "UCS-4-INTERNAL",
                                                       (char *) info->manufacturer_string,
                                                       (SDL_wcslen(info->manufacturer_string) + 1) *
                                                       sizeof(wchar_t));
                product_string = SDL_iconv_string("UTF-8", "UCS-4-INTERNAL",
                                                  (char *) info->product_string,
                                                  (SDL_wcslen(info->product_string) + 1) *
                                                  sizeof(wchar_t));
            }
        ...
        if (manufacturer_string && product_string) {
            size_t name_size = (SDL_strlen(manufacturer_string) + 1 + SDL_strlen(product_string) +
                                1);
...
HIDAPI_JoystickOpen(SDL_Joystick *joystick, int device_index) {
    ...
    if (!device->driver->Init(joystick, hwdata->dev, device->vendor_id, device->product_id,
                              &hwdata->context)) {
...
HIDAPI_JoystickRumble(SDL_Joystick *joystick, Uint16 low_frequency_rumble,
    ...
    result = driver->Rumble(joystick, hwdata->dev, hwdata->context, low_frequency_rumble,
