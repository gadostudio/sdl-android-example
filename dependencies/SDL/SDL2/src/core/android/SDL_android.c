...
int Android_JNI_SetupThread(void) {
    ...
    status = (*mJavaVM)->AttachCurrentThread(mJavaVM, &env, NULL);
    if (status < 0) {
        __android_log_print(ANDROID_LOG_ERROR, "SDL", "Failed to attach current thread (err=%d)",
                            status);
        return 0;
    }
...
JNIEXPORT void JNICALL SDL_JAVA_INTERFACE(nativeSetupJNI)(JNIEnv *env, jclass cls) {
    ...
    midGetDisplayDPI = (*env)->GetStaticMethodID(env, mActivityClass, "getDisplayDPI",
                                                 "()Landroid/util/DisplayMetrics;");
    midCreateCustomCursor = (*env)->GetStaticMethodID(env, mActivityClass, "createCustomCursor",
                                                      "([IIIII)I");
    midSetCustomCursor = (*env)->GetStaticMethodID(env, mActivityClass, "setCustomCursor", "(I)Z");
    midSetSystemCursor = (*env)->GetStaticMethodID(env, mActivityClass, "setSystemCursor", "(I)Z");

    midSupportsRelativeMouse = (*env)->GetStaticMethodID(env, mActivityClass,
                                                         "supportsRelativeMouse", "()Z");
    midSetRelativeMouseEnabled = (*env)->GetStaticMethodID(env, mActivityClass,
                                                           "setRelativeMouseEnabled", "(Z)Z");


    if (!midGetNativeSurface || !midSetSurfaceViewFormat ||
        !midSetActivityTitle || !midSetWindowStyle || !midSetOrientation || !midMinimizeWindow ||
        !midShouldMinimizeOnFocusLoss || !midGetContext || !midIsTablet || !midIsAndroidTV ||
        !midInitTouch ||
        !midSendMessage || !midShowTextInput || !midIsScreenKeyboardShown ||
        !midClipboardSetText || !midClipboardGetText || !midClipboardHasText ||
        !midOpenAPKExpansionInputStream || !midGetManifestEnvironmentVariables ||
        !midGetDisplayDPI ||
        !midCreateCustomCursor || !midSetCustomCursor || !midSetSystemCursor ||
        !midSupportsRelativeMouse || !midSetRelativeMouseEnabled ||
...
JNIEXPORT void JNICALL SDL_JAVA_AUDIO_INTERFACE(nativeSetupJNI)(JNIEnv *env, jclass cls) {
    ...
    if (!midAudioOpen || !midAudioWriteByteBuffer || !midAudioWriteShortBuffer ||
        !midAudioWriteFloatBuffer || !midAudioClose ||
...
SDL_JAVA_INTERFACE(nativeRunMain)(JNIEnv *env, jclass cls, jstring library, jstring function,
                                  jobject array) {
    int status = -1;
    ...
    if (library_handle) {
        ...
        if (SDL_main) {
            ...
            __android_log_print(ANDROID_LOG_ERROR, "SDL",
                                "nativeRunMain(): Couldn't find function %s in library %s",
                                function_name, library_file);
        }
        ...
        __android_log_print(ANDROID_LOG_ERROR, "SDL", "nativeRunMain(): Couldn't load library %s",
                            library_file);
    }
...
JNIEXPORT void JNICALL SDL_JAVA_INTERFACE(nativeSetScreenResolution)(
    ...
    Android_SetScreenResolution(surfaceWidth, surfaceHeight, deviceWidth, deviceHeight, format,
                                rate);
...
JNIEXPORT jint JNICALL SDL_JAVA_CONTROLLER_INTERFACE(nativeAddJoystick)(
    ...
    retval = Android_AddJoystick(device_id, name, desc, vendor_id, product_id,
                                 is_accelerometer ? SDL_TRUE : SDL_FALSE, button_mask, naxes, nhats,
                                 nballs);
...
JNIEXPORT void JNICALL SDL_JAVA_INTERFACE(nativeFocusChanged)(
    ...
    if (Android_Window) {
        ...
        SDL_SendWindowEvent(Android_Window,
                            (hasFocus ? SDL_WINDOWEVENT_FOCUS_GAINED : SDL_WINDOWEVENT_FOCUS_LOST),
                            0, 0);
    }
...
void Android_JNI_SetOrientation(int w, int h, int resizable, const char *hint) {
    ...
    (*env)->CallStaticVoidMethod(env, mActivityClass, midSetOrientation, w, h, (resizable ? 1 : 0),
                                 jhint);
...
int Android_JNI_OpenAudioDevice(int iscapture, SDL_AudioSpec *spec) {
    ...
    if (iscapture) {
        ...
        result = (*env)->CallStaticObjectMethod(env, mAudioManagerClass, midCaptureOpen, spec->freq,
                                                audioformat, spec->channels, spec->samples);
    } else {
        __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "SDL audio: opening device for output");
        result = (*env)->CallStaticObjectMethod(env, mAudioManagerClass, midAudioOpen, spec->freq,
                                                audioformat, spec->channels, spec->samples);
    }
    if (result == NULL) {
    ...
    if ((*env)->GetArrayLength(env, (jintArray) result) != 4) {
        return SDL_SetError("Unexpected results from Java, expected 4, got %d",
                            (*env)->GetArrayLength(env, (jintArray) result));
    }
    isCopy = JNI_FALSE;
    ...
    switch (audioformat) {
        case ENCODING_PCM_8BIT: {
            ...
            break;
        case ENCODING_PCM_16BIT: {
            jshortArray audioBufferLocal = (*env)->NewShortArray(env,
            ...
            break;
        case ENCODING_PCM_FLOAT: {
            jfloatArray audioBufferLocal = (*env)->NewFloatArray(env,
                                                                 spec->samples * spec->channels);
            if (audioBufferLocal) {
                jbufobj = (*env)->NewGlobalRef(env, audioBufferLocal);
                (*env)->DeleteLocalRef(env, audioBufferLocal);
    ...
    if (!iscapture) {
        ...
        switch (audioformat) {
            ...
            case ENCODING_PCM_16BIT:
                ...
                break;
            case ENCODING_PCM_FLOAT:
                audioBufferPinned = (*env)->GetFloatArrayElements(env, (jfloatArray) audioBuffer,
                                                                  &isCopy);
                break;
            default:
                return SDL_SetError("Unexpected audio format from Java: %d\n", audioformat);
...
void Android_JNI_WriteAudioBuffer(void) {
    ...
    switch (audioBufferFormat) {
        case ENCODING_PCM_8BIT:
            (*env)->ReleaseByteArrayElements(env, (jbyteArray) audioBuffer,
                                             (jbyte *) audioBufferPinned, JNI_COMMIT);
            (*env)->CallStaticVoidMethod(env, mAudioManagerClass, midAudioWriteByteBuffer,
                                         (jbyteArray) audioBuffer);
            break;
        case ENCODING_PCM_16BIT:
            (*env)->ReleaseShortArrayElements(env, (jshortArray) audioBuffer,
                                              (jshort *) audioBufferPinned, JNI_COMMIT);
            (*env)->CallStaticVoidMethod(env, mAudioManagerClass, midAudioWriteShortBuffer,
                                         (jshortArray) audioBuffer);
            break;
        case ENCODING_PCM_FLOAT:
            (*env)->ReleaseFloatArrayElements(env, (jfloatArray) audioBuffer,
                                              (jfloat *) audioBufferPinned, JNI_COMMIT);
...
int Android_JNI_CaptureAudioBuffer(void *buffer, int buflen) {
    ...
    switch (captureBufferFormat) {
        case ENCODING_PCM_8BIT:
            ...
            br = (*env)->CallStaticIntMethod(env, mAudioManagerClass, midCaptureReadByteBuffer,
                                             (jbyteArray) captureBuffer, JNI_TRUE);
            if (br > 0) {
                jbyte *ptr = (*env)->GetByteArrayElements(env, (jbyteArray) captureBuffer, &isCopy);
                SDL_memcpy(buffer, ptr, br);
                (*env)->ReleaseByteArrayElements(env, (jbyteArray) captureBuffer, (jbyte *) ptr,
                                                 JNI_ABORT);
            }
            break;
        case ENCODING_PCM_16BIT:
            SDL_assert((*env)->GetArrayLength(env, (jshortArray) captureBuffer) ==
                       (buflen / sizeof(Sint16)));
            br = (*env)->CallStaticIntMethod(env, mAudioManagerClass, midCaptureReadShortBuffer,
                                             (jshortArray) captureBuffer, JNI_TRUE);
            if (br > 0) {
                jshort *ptr = (*env)->GetShortArrayElements(env, (jshortArray) captureBuffer,
                                                            &isCopy);
                br *= sizeof(Sint16);
                SDL_memcpy(buffer, ptr, br);
                (*env)->ReleaseShortArrayElements(env, (jshortArray) captureBuffer, (jshort *) ptr,
                                                  JNI_ABORT);
            }
            break;
        case ENCODING_PCM_FLOAT:
            SDL_assert((*env)->GetArrayLength(env, (jfloatArray) captureBuffer) ==
                       (buflen / sizeof(float)));
            br = (*env)->CallStaticIntMethod(env, mAudioManagerClass, midCaptureReadFloatBuffer,
                                             (jfloatArray) captureBuffer, JNI_TRUE);
            if (br > 0) {
                jfloat *ptr = (*env)->GetFloatArrayElements(env, (jfloatArray) captureBuffer,
                                                            &isCopy);
                br *= sizeof(float);
                SDL_memcpy(buffer, ptr, br);
                (*env)->ReleaseFloatArrayElements(env, (jfloatArray) captureBuffer, (jfloat *) ptr,
                                                  JNI_ABORT);
            }
            break;
        default:
...
void Android_JNI_FlushCapturedAudio(void) {
    ...
    switch (captureBufferFormat) {
        case ENCODING_PCM_8BIT:
            ...
            break;
        case ENCODING_PCM_16BIT:
            (*env)->CallStaticIntMethod(env, mAudioManagerClass, midCaptureReadShortBuffer,
                                        (jshortArray) captureBuffer, JNI_FALSE);
            break;
        case ENCODING_PCM_FLOAT:
            (*env)->CallStaticIntMethod(env, mAudioManagerClass, midCaptureReadFloatBuffer,
                                        (jfloatArray) captureBuffer, JNI_FALSE);
            break;
        default:
            __android_log_print(ANDROID_LOG_WARN, "SDL",
...
void Android_JNI_AudioSetThreadPriority(int iscapture, int device_id) {
    JNIEnv *env = Android_JNI_GetEnv();
    (*env)->CallStaticVoidMethod(env, mAudioManagerClass, midAudioSetThreadPriority, iscapture,
                                 device_id);
}
    ...
    if (exception != NULL) {
        ...
        if (!silent) {
            ...
            if (exceptionMessage != NULL) {
                const char *exceptionMessageUTF8 = (*env)->GetStringUTFChars(env, exceptionMessage,
                                                                             0);
...
static int Internal_Android_JNI_FileOpen(SDL_RWops *ctx) {
    ...
    */
    mid = (*env)->GetMethodID(env, (*env)->GetObjectClass(env, assetManager), "openFd",
    ...
    mid = (*env)->GetMethodID(env, (*env)->GetObjectClass(env, inputStream), "getStartOffset",
                              "()J");
    ...
    mid = (*env)->GetMethodID(env, (*env)->GetObjectClass(env, inputStream), "getDeclaredLength",
                              "()J");
    ...
    mid = (*env)->GetMethodID(env, (*env)->GetObjectClass(env, inputStream), "getFileDescriptor",
    ...
    if (0) {
        ...
        if (Android_JNI_ExceptionOccurred(SDL_FALSE)) {
            ...
            inputStream = (*env)->CallStaticObjectMethod(env, mActivityClass,
...
int Android_JNI_GetPowerInfo(int *plugged, int *charged, int *battery, int *seconds, int *percent) {
    ...
    mid = (*env)->GetMethodID(env, mActivityClass, "registerReceiver",
...
void Android_JNI_HapticRun(int device_id, float intensity, int length) {
    JNIEnv *env = Android_JNI_GetEnv();
    (*env)->CallStaticVoidMethod(env, mControllerManagerClass, midHapticRun, device_id, intensity,
                                 length);
}
...
int Android_JNI_CreateCustomCursor(SDL_Surface *surface, int hot_x, int hot_y) {
    ...
    if (pixels) {
        ...
        custom_cursor = (*env)->CallStaticIntMethod(env, mActivityClass, midCreateCustomCursor,
                                                    pixels, surface->w, surface->h, hot_x, hot_y);
...
SDL_bool Android_JNI_SetRelativeMouseEnabled(SDL_bool enabled) {
    JNIEnv *env = Android_JNI_GetEnv();
    return (*env)->CallStaticBooleanMethod(env, mActivityClass, midSetRelativeMouseEnabled,
                                           (enabled == 1));
}
