...
SDL_AddFinger(SDL_Touch *touch, SDL_FingerID fingerid, float x, float y, float pressure) {
    ...
    if (touch->num_fingers == touch->max_fingers) {
        ...
        new_fingers = (SDL_Finger **) SDL_realloc(touch->fingers, (touch->max_fingers + 1) *
                                                                  sizeof(*touch->fingers));
        if (!new_fingers) {
            return SDL_OutOfMemory();
        }
                ...
                if (window) {
                    if (down) {
                        if (finger_touching == SDL_FALSE) {
                            ...
                            SDL_SendMouseButton(window, SDL_TOUCH_MOUSEID, SDL_PRESSED,
                                                SDL_BUTTON_LEFT);
                        }
                    } else {
                        if (finger_touching == SDL_TRUE && track_touchid == id &&
                            track_fingerid == fingerid) {
                            SDL_SendMouseButton(window, SDL_TOUCH_MOUSEID, SDL_RELEASED,
                                                SDL_BUTTON_LEFT);
                        }
                    }
                }
